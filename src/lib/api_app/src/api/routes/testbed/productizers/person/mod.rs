...
fn build_data_product_uri(data_product: &str, data_source: &str) -> String {
    let testbed_base_url: String = env::var("TESTBED_BASE_URL").expect("TESTBED_BASE_URL must be set");
    let testbed_environment: String = env::var("TESTBED_ENVIRONMENT").expect("TESTBED_ENVIRONMENT must be set");
    let path: String = testbed_base_url + data_product + "?source=" + data_source + ":" + testbed_environment.as_str();
    path
}
